...
export const getBookmarks = async (userId) => {
  try {
    const bookmarksDoc = doc(db,"bookmarks",userId);
    const docSnapshot = await getDoc(bookmarksDoc);
    ...
    if(docSnapshot.exists()){
      ...
      throw new Error('User bookmarks not found');
    }
  } catch(error){
    throw new Error(error.message);
  }
...
export const deleteBookmark = async (fav, userId) => {
  try {
    const bookmarksDoc = doc(db, "bookmarks", userId);
    const docSnapshot = await getDoc(bookmarksDoc);
...
export const getFavourites = async (userId) => {
  try {
    ...
    if(docSnapshot.exists()){
      ...
      throw new Error('User favourites not found');
    }
  } catch(error){
    throw new Error(error.message);
  }
